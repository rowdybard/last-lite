...
import { PlayerPersistencePostgres } from '../src/systems/player-persistence-postgres';
import { DatabaseInitializer } from '../src/database/init';
import { Player } from '../shared/types';

describe('PlayerPersistencePostgres', () => {
  let playerPersistence: PlayerPersistencePostgres;
  ...
  beforeEach(async () => {
    ...
    playerPersistence = new PlayerPersistencePostgres();
  ...
  afterEach(async () => {
    // Clean up test data
    await playerPersistence.deletePlayer(mockPlayer.id);
    await playerPersistence.deletePlayer('player-2');
    await playerPersistence.deletePlayer('player-3');
    await dbInitializer.close();
  });

  describe('player saving', () => {
    it('should save player data to PostgreSQL', async () => {
  ...
  describe('backup and restore', () => {
    it('should create backup of player data', async () => {
      ...
      expect(backup.playerCount).toBeGreaterThanOrEqual(1);
    });
